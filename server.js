...
app.use(cors());
app.use(express.static(path.join(__dirname, 'public')));// NS account ID with underscores → hyphens for subdomain
const ACCOUNT_ID = (process.env.NS_ACCOUNT_ID || '').toLowerCase().replace(/_/g, '-');
// Allow NS_BASE_URL override in case auto-generated URL doesn't resolve
...
const enc = s => encodeURIComponent(String(s));function oauthHeader(method, baseUrl, queryParams = {}) {
